...
const PhcServiceAdmin = () => {
  ...
  const { mutate: updatePhcServiceAdminStatus } = useMutationAction(
  ...
  const handleSwitchStatus = (phcServiceAdmin: IUser) => {
    showAlert({
      ...
      onConfirm: () => {
        handleSwitchStatusConfirm(phcServiceAdmin.id, !phcServiceAdmin.enabled);
      },
    });
  };
      ...
      (phcServiceAdmins?.data || []).map((phcServiceAdmin) => {
        ...
        return {
          ...
          last_login: phcServiceAdmin.last_login
            ? moment
                .utc(phcServiceAdmin.last_login)
                .local()
  ...
  return (
    <div className="mt-3">
      <p>{t('common.phc_service_admin.management')}</p>
      <BasicTable
        showSearch={true}
        showPagination={true}
