...
const useQuestionSkipLogic = (question:any) => {
  const { control, unregister } = useFormContext();

  const fieldNames = useMemo(
    () =>
      question.logics?.map((logic:any) => getQuestionName(logic.target_question_id)) ?? [],
    [question.logics]
  );
  ...
  const shouldSkip = useMemo(() => {
    ...
    return !question.logics.every((logic:any, index:number) =>
...
const NoteRender = ({ question }: { question: any }) => {
...
const RadioRender = ({ question, disabled, translate }: { question: any, disabled: boolean, translate: any }) => {
  const {
    field,
    fieldState: { error },
  } = useController({
...
const CheckBoxRender = ({ question, disabled, translate }: { question: any, disabled: boolean, translate: any }) => {
  const {
    field,
    fieldState: { error },
  } = useController({
        ...
        {question.options.map((opt:any) => {
          ...
          const handleChange = () => {
            ...
            if (current.includes(opt.id)) {
              field.onChange(current.filter((id:any) => id !== opt.id));
            } else {
...
const InputTextRender = ({ question, disabled, translate }: { question: any, disabled: boolean, translate: any }) => {
  const {
    field,
    fieldState: { error },
  } = useController({
...
const InputNumberRender = ({ question, disabled, translate }: { question: any, disabled: boolean, translate: any }) => {
...
const SliderRender = ({ question, disabled, translate }: { question: any, disabled: boolean, translate: any }) => {
  const {
    field
  } = useController({
    name: getQuestionName(question.id),
...
const QuestionRenderer = ({ question, disabled }:{question:any, disabled:boolean}) => {
  ...
  const Component = componentMap[question.question_type as keyof typeof componentMap] ?? null;
